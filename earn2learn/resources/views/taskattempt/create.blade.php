...
@section('content')
    ...
    <h1>Create New Taskattempt</h1>
    <hr/>

    {!! Form::open(['url' => 'taskattempt', 'class' => 'form-horizontal']) !!}
            ...
            <div class="form-group {{ $errors->has('allowcation_id') ? 'has-error' : ''}}">
                {!! Form::label('allowcation_id', 'Allowcation Id: ', ['class' => 'col-sm-3 control-label']) !!}
                <div class="col-sm-6">
                    {!! Form::number('allowcation_id', null, ['class' => 'form-control']) !!}
                    {!! $errors->first('allowcation_id', '<p class="help-block">:message</p>') !!}
                </div>
            </div>
